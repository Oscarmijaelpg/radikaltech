...
export interface ImageVisualAnalysis {
  ...
  style_tags: string[];
  description: string;
  creative_prompt_summary: string; // Resumen técnico para inyectar en generadores
}

const PROMPT =
  'Analiza esta imagen como director de arte experto. Devuelve SOLO JSON con { dominant_colors: array 3-5 hex, lighting, mood, composition, style_tags: array, description: texto descriptivo, creative_prompt_summary: un párrafo técnico de 2-3 líneas que describa exactamente cómo recrear esta estética (iluminación, texturas, ángulos, atmósfera) para un modelo de generación de imágenes }. No incluyas texto fuera del JSON.';
...
function safeParse(content: string): ImageVisualAnalysis | null {
  ...
  try {
    ...
    return {
      ...
      creative_prompt_summary: typeof parsed.creative_prompt_summary === 'string' ? parsed.creative_prompt_summary : '',
    };
  } catch (err) {
