...
async function downloadAndStoreImage(
  ...
  try {
    ...
    const storagePath = `files/images/analysis_${randomUUID()}.${ext}`;
        ...
        if (uniqueImages.length > 0) {
          ...
          for (const imgUrl of uniqueImages) {
            try {
              ...
              if (stored) {
                // Analizar la imagen inmediatamente para capturar el ADN visual
                const analysis = await this.imageAnalyzer.analyze(stored.publicUrl);
