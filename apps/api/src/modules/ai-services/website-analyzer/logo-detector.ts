...
// SVGs are intentionally penalized: they cannot be used by the AI for generation or watermarking
const BONUS_EXT_SVG = -50;
...
async function tryDownloadOne(
  ...
  try {
    ...
    // SVGs are NOT usable for AI generation or watermarking — skip and try next candidate
    if (ext === 'svg' || logoUrl.toLowerCase().endsWith('.svg') || (ct && ct.includes('svg'))) {
      if (jl) await jl.warn(`Logo SVG ignorado (no compatible con IA): ${logoUrl.split('/').pop()}`);
      return undefined;
    }
    ...
    if (projectId) {
      const asset = await prisma.contentAsset.create({
        data: {
          ...
          assetType: 'image',
          assetUrl: publicUrl,
          aiDescription: tags.includes('logo_candidate') ? 'Candidato a logo detectado automáticamente' : 'Logo detectado automáticamente',
          tags,
          metadata: {
            source: 'website_auto',
            origin_url: logoUrl,
...
export async function downloadAndStoreLogo(
  ...
  let mainLogoResult: { publicUrl: string; assetId?: string } | undefined = undefined;
  let candidatesFound = 0;

  for (const c of candidates) {
    // Top 1 valid becomes the main logo, up to 3 more become candidates
    if (!mainLogoResult) {
      const result = await tryDownloadOne(c, userId, projectId, ['logo', 'website_auto'], jl);
      if (result) mainLogoResult = result;
    } else {
      if (candidatesFound >= 3) break;
      const result = await tryDownloadOne(c, userId, projectId, ['logo_candidate', 'website_auto'], jl);
      if (result) candidatesFound++;
    }
  }
  return mainLogoResult;
}
