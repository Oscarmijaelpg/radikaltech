...
export class ImageGenerator {
  async generate(input: GenerateImageInput): Promise<GenerateImageOutput> {
    ...
    const enrichedPrompt = await buildBrandContext(input.projectId, useBrandPalette, input.prompt, mode);
    ...
    try {
      ...
      const runOne = async (idx: number): Promise<GeneratedVariation | null> => {
        ...
        let lastError = '';
        if (refs.length > 0) {
          logger.info({ model: LLM_MODELS.image.geminiDefault, refs: refs.length }, 'attempting gemini generation');
          const gen = await generateWithGemini(variantPrompt, refs);
          buf = gen.buffer;
          lastError = gen.error || '';
      ...
      if (job) {
        try {
          ...
          logger.warn({ err }, 'failed to update aiJob status to succeeded');
        }
      }
      ...
      if (job) {
        try {
          ...
          logger.warn({ err: jErr }, 'failed to update aiJob status to failed');
        }
      }
      try {
  ...
  async edit(input: EditImageInput): Promise<EditImageOutput> {
    ...
    try {
      ...
      logger.warn({ err }, 'failed to create aiJob (edit), continuing');
    }
    ...
    try {
      ...
      if (refs.length > 0) {
        const gen = await generateWithGemini(enrichedPrompt, refs);
        buf = gen.buffer;
        if (buf) modelUsed = LLM_MODELS.image.geminiDefault;
      }
      if (!buf || !modelUsed) {
      ...
      if (job) {
        try {
          ...
          logger.warn({ err }, 'failed to update aiJob (edit) to succeeded');
        }
      }
      ...
      if (job) {
        try {
          ...
          logger.warn({ err: jErr }, 'failed to update aiJob (edit) to failed');
        }
      }
      try {
        await notificationService
