...
import { runVisualAnalysisForCompetitor, downloadAndStoreImage } from './instagram-scraper.js';
  ...
  async scrape(input: ScrapeTikTokInput): Promise<ScrapeTikTokResult> {
    ...
    try {
      ...
      for (const item of Array.isArray(items) ? items : []) {
        ...
        // Analizar la portada para capturar el ADN visual funcional
        if (coverUrl) {
          try {
            const analysis = await imageAnalyzer.analyze(coverUrl);
            await prisma.contentAsset.create({
              data: {
                ...
                tags: ['tiktok', 'social_auto'],
                metadata: {
                  source: 'tiktok_scrape',
                  post_url: postUrl,
                  caption,
                  published_at: publishedAt,
                  visual_analysis: analysis || undefined
                } as any,
              },
            });
          } catch (e) { /* ignore */ }
