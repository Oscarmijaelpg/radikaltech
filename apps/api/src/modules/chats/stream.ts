...
export async function handleStreamMessage(c: Context<{ Variables: AuthVariables }>) {
  ...
  const history: ChatMsg[] = recent
    ...
    .map((m) => {
      ...
      // Inject tool results (like generated images) into the assistant's memory
      if (m.role === 'assistant' && m.metadata && typeof m.metadata === 'object') {
        const meta = m.metadata as { tools?: Array<{ name: string; resultSummary?: string; data?: any }> };
        if (meta.tools && Array.isArray(meta.tools)) {
          for (const t of meta.tools) {
            if ((t.name === 'generate_image' || t.name === 'edit_image') && t.data?.url) {
              textContent += `\n[Recurso Visual Generado: ${t.data.url}]`;
            } else if (t.resultSummary) {
              // Optionally inject other tool summaries if needed, but primarily images for now
              // textContent += `\n[Acción realizada: ${t.name} - ${t.resultSummary}]`;
            }
          }
        }
      }
      ...
      try {
        ...
        for (let round = 0; round <= MAX_TOOL_ROUNDS; round++) {
          ...
          for (const tc of pendingToolCalls) {
            ...
            toolLog.push({ name: tc.name, args: parsedArgs, resultSummary: result.summary, data: result.data });
            send(
              'tool_result',
              JSON.stringify({
                name: tc.name,
