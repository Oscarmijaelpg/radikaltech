...
export const generateImageTool: ToolDefinition = {
  ...
  async execute(args, ctx) {
    ...
    return {
      summary: first ? 'Imagen generada con éxito.' : 'No se pudo generar la imagen.',
...
export const editImageTool: ToolDefinition = {
  schema: {
    ...
    function: {
      name: 'edit_image',
      description:
        'Modifica una imagen existente basándose en una instrucción específica (ej: "quita a la mujer", "hazlo más soleado"). Úsalo SOLO cuando el usuario pida cambios sobre una imagen que ya se mostró en el chat.',
      parameters: {
        type: 'object',
        properties: {
          source_asset_id: {
            type: 'string',
            description: 'ID de la imagen original a modificar.',
          },
          instruction: {
            type: 'string',
            description: 'Instrucción detallada del cambio a realizar.',
          },
        },
  ...
  label: 'Editando imagen',
  async execute(args, ctx) {
    const assetId = String(args.source_asset_id ?? '').trim();
    const instruction = String(args.instruction ?? '').trim();
    if (!assetId || !instruction) return { summary: 'Faltan argumentos', error: 'missing_args' };

    const out = await imageGenerator.edit({
    ...
    return {
      summary: 'Imagen editada con éxito.',
      data: { url: out.url, assetId: out.assetId },
    };
  },
};
...
export const getLibraryAssetsTool: ToolDefinition = {
  schema: {
    ...
    function: {
      ...
      description:
        'Obtener y MOSTRAR la biblioteca visual de la marca (logos, fotos, referencias). Úsalo para que el usuario elija referencias ANTES de una generación nueva. NO lo uses si el usuario pide editar una imagen existente.',
      parameters: {
        type: 'object',
        properties: {
          concept: {
  ...
  async execute(args, ctx) {
    ...
    const otherAssets = await prisma.contentAsset.findMany({
      where: {
        ...
        // Exclude SVGs — they are not compatible with generative AI or display
        NOT: { assetUrl: { endsWith: '.svg' } },
      },
