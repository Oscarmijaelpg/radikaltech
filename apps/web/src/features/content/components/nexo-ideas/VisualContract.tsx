...
export function VisualContract({ idea, open, onOpenChange, onSuccess }: Props) {
  ...
  const { data: assets, isLoading: isLoadingAssets } = useAssets(activeProject?.id, { type: 'image', limit: 100 });
  const generate = useGenerateImage();

  const logos = useMemo(() => 
    assets?.filter(a => a.tags.includes('logo') && !a.asset_url.toLowerCase().endsWith('.svg')) || []
  , [assets]);

  const allPossibleRefs = useMemo(() => {
    if (!assets) return [];
    return assets.filter(a => {
      // Don't show the main logo in the "Sira References" grid since it's in the "Brand Identity" section
      if (a.tags.includes('logo')) return false;
      // Filter out SVGs
      if (a.asset_url.toLowerCase().endsWith('.svg')) return false;
      
      const relevantTags = [
        'instagram', 'web', 'reference', 'social_auto', 'website_auto', 
        'moodboard', 'product', 'social_media'
      ];
      return a.tags.some(t => relevantTags.includes(t));
    });
  }, [assets]);

  // Sira's Recommended Assets based on Idea keywords
  const recommendedAssets = useMemo(() => {
    if (!idea || !allPossibleRefs.length) return [];
    const text = (idea.visual_suggestion + ' ' + idea.title).toLowerCase();
    
    return allPossibleRefs.filter(a => {
      const desc = (a.ai_description || '').toLowerCase();
      const tags = a.tags.join(' ').toLowerCase();
      // Simple keyword matching for "smart" suggestions
      const keywords = ['oficina', 'café', 'nature', 'people', 'minimal', 'dark', 'light', 'interior', 'product'];
      const matchesKeyword = keywords.some(k => text.includes(k) && (desc.includes(k) || tags.includes(k)));
      // Also prioritize high score if available
      return matchesKeyword || a.tags.includes('selected');
    }).slice(0, 4);
  }, [idea, allPossibleRefs]);

  // Effect to auto-select recommendations and logo
  useEffect(() => {
    if (open && idea && assets) {
      ...
      if (selectedRefs.size === 0 && recommendedAssets.length > 0) {
        setSelectedRefs(new Set(recommendedAssets.map(a => a.id)));
      }
    }
  }, [open, idea, assets, logos, recommendedAssets]);
            ...
            <div className="flex-1 overflow-y-auto p-8 pt-6 space-y-10 scroll-smooth">
              <div className="space-y-10">
                ...
                <div>
                  ...
                  ) : allPossibleRefs.length > 0 ? (
                    <div className="grid grid-cols-4 gap-2">
                      {allPossibleRefs.map(ref => {
                        const isSelected = selectedRefs.has(ref.id);
                        const isRecommended = recommendedAssets.some(r => r.id === ref.id);
                        ...
                        return (
                          ...
                          >
                            ...
                            {isRecommended && (
                              <div className="absolute bottom-1 left-1 px-1 rounded bg-amber-500/90 text-white text-[7px] font-black uppercase tracking-tighter shadow-sm">
                                Sugerido
                              </div>
                            )}
                            {isSelected && (
