...
export function OnboardingPage() {
    ...
    const { state, loading, saving, error, dismissError, submitStep, goNext, goBack, goToStep, completeOnboarding, } = useOnboarding();
    const stepperIndex = useMemo(() => {
        if (state.currentStep === 'company') {
            ...
            const hasCompanyBasics = state.company?.company_name && state.company?.industry;
            if (hasCompanyBasics &&
    ...
    if (loading) {
        return (_jsx("div", { className: "min-h-screen grid place-items-center bg-gradient-to-br from-pink-50 via-white to-cyan-50", children: _jsx(Spinner, { size: "lg" }) }));
    }
    ...
    const handleCompanyBasics = async (partial) => {
        await submitStep('company', partial);
    };
    const handleCompanyFull = async (full) => {
        ...
        if (full.website_source === WebsiteSource.URL && full.website_url && full.website_url !== prevUrl) {
            toast({
                ...
                description: 'Sira está extrayendo logo, colores y propuesta de valor. Aparecerán automáticamente en los próximos pasos.',
                variant: 'success',
            });
    ...
    const character = (() => {
        ...
        if (state.currentStep === 'company' && !showWebsiteSubstep) {
            return {
                src: ankorImg,
                name: 'Ankor',
                message: 'Necesitamos estos datos para darle una identidad formal a tu empresa. Será el ancla de todo lo que creemos juntos.',
            };
        }
        if (state.currentStep === 'company' && showWebsiteSubstep) {
            return {
                src: siraImg,
                name: 'Sira',
                message: 'Si tienes un sitio, puedo analizarlo y mapear tu negocio en segundos. Si no, cuéntame tú a qué te dedicas.',
            };
        }
        if (state.currentStep === 'socials') {
            return {
                src: nexoImg,
                name: 'Nexo',
                message: 'Conecta tus redes — tantas como quieras, ninguna es obligatoria. Las uso para aprender tu voz visual.',
            };
        }
        if (state.currentStep === 'brand') {
            return {
                src: kronosImg,
                name: 'Kronos',
                message: 'Tu voz de marca es tu firma. Cuéntame cómo suenas para que todo lo que escriba se sienta 100% tuyo.',
            };
        }
        if (state.currentStep === 'objectives') {
            return {
                src: indexaImg,
                name: 'Indexa',
                message: 'Con objetivos claros puedo medir lo que importa y orientar cada propuesta hacia resultados reales.',
            };
        }
        return undefined;
    })();
    return (_jsxs(OnboardingLayout, { stepIndex: stepperIndex, showStepper: state.currentStep !== 'welcome' && state.currentStep !== 'complete', character: character, fullWidth: state.currentStep === 'welcome' || state.currentStep === 'complete', errorMessage: error, onDismissError: dismissError, saving: saving, children: [state.currentStep === 'welcome' && (_jsx(WelcomeStep, { onStart: () => goToStep('company') })), state.currentStep === 'company' && !showWebsiteSubstep && (_jsx(CompanyStep, { defaultValues: state.company, saving: saving, onBack: () => goToStep('welcome'), onSubmit: handleCompanyBasics })), state.currentStep === 'company' && showWebsiteSubstep && (_jsx(WebsiteStep, { defaultValues: {
                    ...
                    goToStep('welcome');
                }, onSubmit: handleCompanyFull })), state.currentStep === 'socials' && (_jsx(SocialStep, { defaultValues: state.socials, saving: saving, onBack: goBack, onSubmit: async (data) => {
                    await submitStep('socials', data);
                    const hasIgUrl = data.accounts.some((a) => a.platform === 'instagram' && a.source === 'url' && a.url);
                    if (hasIgUrl) {
                        toast({
                            title: 'Extrayendo tu Instagram',
                            description: 'Estamos descargando tus últimos posts para aprender tu voz visual. Puedes seguir avanzando mientras termina.',
                            variant: 'success',
                        });
                        qc.invalidateQueries({ queryKey: ['jobs', 'active', 'user'] });
                    }
                    goNext();
                } })), state.currentStep === 'brand' && (_jsx(BrandStep, { defaultValues: state.brand, saving: saving, onBack: goBack, onSubmit: async (data) => {
                    await submitStep('brand', data);
                    goNext();
                } })), state.currentStep === 'objectives' && (_jsx(ObjectivesStep, { defaultValues: state.objectives, saving: saving, brandCompleted: !!state.brand?.tone_of_voice, onBack: goBack, onSubmit: async (data) => {
                    await submitStep('objectives', data);
                    goNext();
                } })), state.currentStep === 'complete' && (_jsx(CompleteStep, { state: state, onFinish: async () => {
