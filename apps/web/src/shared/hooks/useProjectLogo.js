...
export function useProjectLogo(projectId) {
    const result = useProjectLogoWithBrightness(projectId);
    return result.url;
}
export function useProjectLogoWithBrightness(projectId) {
    const { data } = useQuery({
    ...
    const [brightness, setBrightness] = useState('unknown');
    useEffect(() => {
        if (!url) {
...
export function useLogoBrightness(url) {
    const [brightness, setBrightness] = useState('unknown');
    useEffect(() => {
        if (!url) {
            setBrightness('unknown');
...
export function logoContainerStyle(brightness) {
    return {
        backgroundColor: brightness === 'light' ? '#1e293b' :
