...
export type CharacterKey = 'ankor' | 'sira' | 'nexo' | 'kronos' | 'indexa' | 'auto';

const CHAR_IMAGES: Record<Exclude<CharacterKey, 'auto'>, string> = {
  ankor: ankorProfile,
  sira: siraProfile,
  nexo: nexoProfile,
...
const CHAR_ACCENT: Record<Exclude<CharacterKey, 'auto'>, string> = {
...
function resolveCharacter(c: CharacterKey): Exclude<CharacterKey, 'auto'> {
  ...
  return (
    <div className="flex flex-col md:flex-row items-center justify-center gap-6 md:gap-8 py-10 md:py-14 px-6">
      <div className="animate-in fade-in slide-in-from-left-6 duration-500 shrink-0">
        <div
          className={`w-40 md:w-48 aspect-square rounded-[28px] bg-gradient-to-br ${accent} p-[3px] shadow-xl`}
        >
