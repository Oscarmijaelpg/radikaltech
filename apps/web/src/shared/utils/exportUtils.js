...
export const exportToPDF = async (elementId, fileName = 'informe-radikal.pdf', _data) => {
    const originalElement = document.getElementById(elementId);
    if (!originalElement)
        return;
    // Proxy de estilos para interceptar colores oklch() que rompen html2canvas
    const originalGetComputedStyle = window.getComputedStyle;
    const styleProxy = (el, pseudo) => {
    ...
    window.getComputedStyle = styleProxy;
  ...
  `;
    const content = originalElement.cloneNode(true);
...
export const exportToWord = async (content, title = 'Informe Detallado') => {
